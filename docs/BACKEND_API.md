...
All of the following are implemented. The root `README.md` carries the complete
table, including the voice-assistant and embeddings routes.

- `GET /` - API info and status
- `GET /health` - Health check
- `GET /docs` - Interactive API documentation
- `GET|POST /api/categories` - List / create categories
- `POST /api/transcribe` - Transcribe audio, categorize, and check duplicates
- `GET|POST /api/individuals` - List-search / create-update
...
- `GET /api/export` - CSV export (note: registered twice - see the root README)
...
✅ Task 3.0 Complete - Individual Management
- Save/update individuals endpoints
- Interaction history tracking
- Duplicate detection (note: the PRD's >=95 auto-merge threshold was never
  implemented - see the root README)

Later additions beyond the original task list: semantic search over profile
embeddings, and the realtime voice assistant.
